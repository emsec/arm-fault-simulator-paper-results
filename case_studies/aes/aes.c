...
static void __attribute__ ((noinline)) key_expansion(u8* round_key, const u8* Key)
{
    unsigned i, j, k;
    u8 tempa[4];
    for (i = 0; i < 4; ++i)
...
static void add_round_key(u8 round, state_t* state, const u8* round_keys)
{
    u8 i, j;
    for (i = 0; i < 4; ++i)
    {
        for (j = 0; j < 4; ++j)
        {
            (*state)[i][j] ^= round_keys[(round * 16) + (i * 4) + j];
        }
    }
}
...
static void __attribute__ ((noinline)) cipher(state_t* state, const u8* round_keys)
{
    u8 round = 0;
